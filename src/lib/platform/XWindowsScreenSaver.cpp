...
XWindowsScreenSaver::setXScreenSaverActive(bool activated)
{
    if (m_xscreensaverActive != activated) {
        ...
        if (activated) {
            m_events->add_event(Event(EventType::PRIMARY_SCREEN_SAVER_ACTIVATED, m_eventTarget));
        } else {
            m_events->add_event(Event(EventType::PRIMARY_SCREEN_SAVER_DEACTIVATED, m_eventTarget));
        }
    }
}
