...
OSXScreen::onKey(CGEventRef event)
{
	...
	if (eventKind == kCGEventFlagsChanged) {
		...
		if (m_activeModifierHotKey == 0) {
			if (m_modifierHotKeys.count(newMask) > 0) {
                ...
                m_events->add_event(Event(EventType::PRIMARY_SCREEN_HOTKEY_DOWN, getEventTarget(),
		...
		// then generate a hot key up event.
			...
			if (mask != m_activeModifierHotKeyMask) {
                m_events->add_event(Event(EventType::PRIMARY_SCREEN_HOTKEY_UP, getEventTarget(),
                                    HotKeyInfo::alloc(m_activeModifierHotKey)));
				m_activeModifierHotKey     = 0;
				m_activeModifierHotKeyMask = 0;
			}
	...
	if (i != m_hotKeyToIDMap.end()) {
        ...
        m_events->add_event(Event(type, getEventTarget(), HotKeyInfo::alloc(id)));
		return true;
	}
								...
								void* inUserData)
{
	...
	if (kind == kEventSystemUserSessionDeactivated) {
        ...
        events->add_event(Event(EventType::SCREEN_SUSPEND, screen->getEventTarget()));
	}
	else if (kind == kEventSystemUserSessionActivated) {
		LOG((CLOG_DEBUG "user session activated"));
        events->add_event(Event(EventType::SCREEN_RESUME, screen->getEventTarget()));
	}
...
OSXScreen::handlePowerChangeRequest(natural_t messageType, void* messageArg)
{
	...
	case kIOMessageSystemWillSleep:
        ...
        m_events->add_event(Event(EventType::OSX_SCREEN_CONFIRM_SLEEP,
                                  getEventTarget(), messageArg, Event::kDontFreeData));
		return;

	case kIOMessageSystemHasPoweredOn:
		LOG((CLOG_DEBUG "system wakeup"));
        m_events->add_event(Event(EventType::SCREEN_RESUME, getEventTarget()));
		break;
...
OSXScreen::handleConfirmSleep(const Event& event, void*)
{
	...
	if (messageArg != 0) {
		...
		if (m_pmRootPort != 0) {
            ...
            m_events->add_event(Event(EventType::SCREEN_SUSPEND, getEventTarget(),
