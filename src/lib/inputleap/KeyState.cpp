                            ...
                            KeyModifierMask mask, std::int32_t count, KeyButton button)
{
    if (m_keyMap.isHalfDuplex(key, button)) {
        if (isAutoRepeat) {
            ...
            m_events->add_event(Event(EventType::KEY_STATE_KEY_DOWN, target,
                                      KeyInfo::alloc(key, mask, button, 1)));
            m_events->add_event(Event(EventType::KEY_STATE_KEY_UP, target,
                                      KeyInfo::alloc(key, mask, button, 1)));
        }
    }
    else {
        if (isAutoRepeat) {
            m_events->add_event(Event(EventType::KEY_STATE_KEY_REPEAT, target,
