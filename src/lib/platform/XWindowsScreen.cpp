...
XWindowsScreen::fakeMouseWheel(SInt32 xDelta, SInt32 yDelta) const
{
	...
	if (yDelta) { // up/down
		...
		if (numEvents >= 0) {
			xButton = 7; // RIGHT -- hopefully good
		} else {
			xButton = 6; // LEFT -- hopefully good
		}
	}
...
XWindowsScreen::mapButtonFromX(const XButtonEvent* event) const
{
	...
	switch (button) {
		...
		case 8: // browser backwards - mouse4
			return kButtonExtra0;

		case 9: // browser forwards - mouse5
			return kButtonExtra1;

		default: // yeah, just unknown
			return kButtonNone;
	}
}
