...
ClientProxy1_0::disconnect()
{
    removeHandlers();
    getStream()->close();
    m_events->add_event(Event(EventType::CLIENT_PROXY_DISCONNECTED, getEventTarget()));
}
...
bool ClientProxy1_0::parseHandshakeMessage(const std::uint8_t* code)
{
    if (memcmp(code, kMsgCNoop, 4) == 0) {
        ...
        if (recvInfo()) {
            m_events->add_event(Event(EventType::CLIENT_PROXY_READY, getEventTarget()));
            addHeartbeatTimer();
            return true;
        }
    }
...
bool ClientProxy1_0::parseMessage(const std::uint8_t* code)
{
    if (memcmp(code, kMsgDInfo, 4) == 0) {
        if (recvInfo()) {
            m_events->add_event(Event(EventType::SCREEN_SHAPE_CHANGED, getEventTarget()));
            return true;
        }
        return false;
    }
...
ClientProxy1_0::recvGrabClipboard()
{
    ...
    m_events->add_event(Event(EventType::CLIPBOARD_GRABBED, getEventTarget(), info));
